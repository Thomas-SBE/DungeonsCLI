...
public class CMD_InventoryGoBack implements ICommand {
    @Override
    ...
    public boolean execute(String[] argv, Object[] data) {
        GameStateManager.getInstance().closeInventory();
        return true;
    }

    @Override
    public String getHelpDesc() {
        return GlobalUIM.getInstance().filledText(" %sQuitter l'inventaire :%s b %s%s".formatted(Constants.SCOLOR_YELLOW, Constants.SCOLOR_GREEN, Constants.SCOLOR_CYAN, Constants.SCOLOR_RESET), 78, " ", 19);
    }
}
