...
public class CMD_Attack_Spell implements ICommand {
    @Override
    ...
    public boolean valid(String[] argv, Object[] data) {
        try {
            ...
            for(IAbility a : abilities) if(a.getIdentifier().equals(argv[2])) return true;
    ...
    public boolean execute(String[] argv, Object[] data) {
        List<IAbility> abilities = GlobalGameManager.getInstance().player.getAbilities();
        IAbility curr_ability = null;
        for(IAbility a : abilities) if(a.getIdentifier().equals(argv[2])) { curr_ability = a; break; }
        int enemy_index = Integer.parseInt(argv[1]);
        DungeonRoom current_room = (DungeonRoom) data[0];
        ...
        System.out.println(Constants.SCOLOR_BRIGHT + Constants.SCOLOR_WHITE + "│" + GlobalUIM.getInstance().filledText(" • %sVous %sutilisez la compétence %s%s%s !".formatted(Constants.SCOLOR_GREEN, Constants.SCOLOR_WHITE, Constants.SCOLOR_YELLOW, curr_ability.getName(), Constants.SCOLOR_WHITE), 78, " ", 20) + "│");
        ...
        System.out.println(Constants.SCOLOR_BRIGHT + Constants.SCOLOR_WHITE + "│" + GlobalUIM.getInstance().filledText(" • %sVous %sinfligez %s%d%s de dégats à [%s%d%s] %s%s%s !".formatted(Constants.SCOLOR_GREEN, Constants.SCOLOR_WHITE, Constants.SCOLOR_YELLOW, current_room.enemies.get(enemy_index).getDamageOnEntity(curr_ability.getRawDamage() * damage_multiplier), Constants.SCOLOR_WHITE, Constants.SCOLOR_CYAN, enemy_index, Constants.SCOLOR_WHITE, Constants.SCOLOR_RED, current_room.enemies.get(enemy_index).name, Constants.SCOLOR_WHITE), 78, " ", 40) + "│");

        current_room.enemies.get(enemy_index).damage(curr_ability.getRawDamage() * damage_multiplier);
    ...
    public String getHelpDesc() {
        return GlobalUIM.getInstance().filledText(" %sAttaquer ennemi avec un sort :%s a %s<index_ennemi> <spell_id>%s".formatted(Constants.SCOLOR_YELLOW, Constants.SCOLOR_GREEN, Constants.SCOLOR_CYAN, Constants.SCOLOR_RESET), 78, " ", 19);
    }
}
